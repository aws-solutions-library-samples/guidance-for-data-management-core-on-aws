...
import {
	...
	DATA_ASSET_SPOKE_CREATE_REQUEST_EVENT,
	DATA_ASSET_SPOKE_CREATE_RESPONSE_EVENT,
	DATA_ASSET_SPOKE_EVENT_SOURCE,
	DATA_ZONE_DATA_SOURCE_RUN_FAILED,
	DATA_ZONE_DATA_SOURCE_RUN_SUCCEEDED,
	DATA_ZONE_EVENT_SOURCE,
	STEP_FUNCTION_EVENT_SOURCE,
	STEP_FUNCTION_STATUS_CHANGE,
} from '@dm/events';
...
export class DataAsset extends Construct {
	...
	constructor(scope: Construct, id: string, props: DataAssetConstructProperties) {
		...
		const DataZoneAssetReadPolicy = new PolicyStatement({
			actions: ['datazone:GetAsset', 'datazone:ListAssetRevisions', 'datazone:GetDomain'],
			resources: [`*`],
		});

		const DataZoneAssetWritePolicy = new PolicyStatement({
			actions: ['datazone:CreateAssetRevision', 'datazone:CreateAsset', 'datazone:DeleteAsset', 'datazone:CreateListingChangeSet'],
			resources: [`*`],
		});
		...
		const SFNSendTaskPolicy = new PolicyStatement({
			actions: ['states:SendTaskSuccess', 'states:SendTaskFailure', 'iam:PassRole'],
			resources: [`arn:aws:states:${region}:${accountId}:stateMachine:dm-*`],
		});
		...
		const DataZoneProjectPolicy = new PolicyStatement({
			actions: ['datazone:CreateProject', 'datazone:ListProjects', 'datazone:CreateFormType', 'datazone:GetFormType'],
			resources: [`*`],
		});
		...
		startCreateFlowLambdaRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'StartCreateFlowPolicy', 'arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole')
		);
		...
		startCreateFlowAssumedRole.addToPolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
		...
		createDataSourceLambdaRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'CreateDataSourcePolicy', 'arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole')
		);
		...
		createDataSourceRole.addToPolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
				actions: ['sts:AssumeRole', 'sts:TagSession'],
		...
		createDataSourceRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'CreateDataSourceExecutionPolicy', 'arn:aws:iam::aws:policy/service-role/AmazonDataZoneDomainExecutionRolePolicy')
		);
		createDataSourceRole.addToPrincipalPolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
		...
		verifyDataSourceLambdaRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'VerifyDataSourcePolicy', 'arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole')
		);
		...
		verifyDataSourceRole.addToPolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
				actions: ['sts:AssumeRole', 'sts:TagSession'],
		...
		verifyDataSourceRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'VerifyDataSourceExecutionPolicy', 'arn:aws:iam::aws:policy/service-role/AmazonDataZoneDomainExecutionRolePolicy')
		);
		verifyDataSourceRole.addToPrincipalPolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
				actions: ['iam:GetRole', 'iam:GetUser'],
				resources: ['*'],
		...
		runDataSourceLambdaRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'RunDataSourcePolicy', 'arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole')
		);
		...
		runDataSourceRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'RunDataSourceExecutionPolicy', 'arn:aws:iam::aws:policy/service-role/AmazonDataZoneDomainExecutionRolePolicy')
		);
		runDataSourceRole.addToPrincipalPolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
				actions: ['iam:GetRole', 'iam:GetUser'],
				resources: ['*'],
		...
		const dataAssetStateMachineLogGroup = new LogGroup(this, 'DataAssetLogGroup', {
			logGroupName: `/aws/vendedlogs/states/${namePrefix}-dataAsset-create`,
			removalPolicy: RemovalPolicy.DESTROY,
		});
		...
		const dataAssetCreateStateMachine = new StateMachine(this, 'DataAssetCreateStateMachine', {
			definitionBody: DefinitionBody.fromChainable(
				startTask
					...
					.next(
						new Choice(this, 'Data Source is Ready?')
							.when(Condition.stringEquals('$.execution.dataSourceCreation.status', 'READY'), runDataSourceTask.next(lineageTask))
		...
		apiLambdaExecutionRole.addManagedPolicy(
		...
		customDataZoneRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'DZDomainExecutionPolicy', 'arn:aws:iam::aws:policy/service-role/AmazonDataZoneDomainExecutionRolePolicy')
		);
		customDataZoneRole.addToPrincipalPolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
				actions: ['iam:GetRole', 'iam:GetUser'],
				resources: ['*'],
		...
		eventProcessorLambdaRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'EventProcessorLambdaExecutionRole', 'arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole')
		);
		...
		eventProcessorDataZoneRole.addManagedPolicy(
			ManagedPolicy.fromManagedPolicyArn(this, 'EventProcessorExecutionPolicy', 'arn:aws:iam::aws:policy/service-role/AmazonDataZoneDomainExecutionRolePolicy')
		);
		eventProcessorDataZoneRole.addToPrincipalPolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
				actions: ['iam:GetRole', 'iam:GetUser'],
				resources: ['*'],
		...
		new CfnEventBusPolicy(this, 'JobEventBusSubscribePolicy', {
			...
			statement: {
				...
				Resource: [`arn:aws:events:${region}:${accountId}:rule/${dmEventBusName}/*`],
				Principal: '*',
				Condition: {
		...
		NagSuppressions.addResourceSuppressions(
			[
				...
				customDataZoneRole,
				hubEventProcessorLambda,
				startCreateFlowLambdaRole,
				createDataSourceLambdaRole,
				verifyDataSourceLambdaRole,
				runDataSourceLambdaRole,
				eventProcessorLambdaRole,
				eventProcessorDataZoneRole,
				createDataSourceRole,
				verifyDataSourceRole,
				runDataSourceRole,
			],
			[
				{
				...
				},
				{
					...
					appliesTo: [
						...
						'Resource::arn:aws:states:<AWS::Region>:<AWS::AccountId>:stateMachine:dm-*',
						'Action::s3:Abort*',
						'Resource::arn:<AWS::Partition>:s3:::<SsmParameterValuedmsharedbucketNameC96584B6F00A464EAD1953AFF4B05118Parameter>/*',
						'Action::s3:GetBucket*',
						'Action::s3:GetObject*',
						'Action::s3:List*',
		...
		NagSuppressions.addResourceSuppressions(
			[apiLambda, startCreateFlowLambda, createDataSourceLambda, verifyDataSourceLambda, runDataSourceLambda, publishLineageLambda, createProjectLambda],
			[
				{
					id: 'AwsSolutions-L1',
					reason: 'Latest runtime not needed.',
