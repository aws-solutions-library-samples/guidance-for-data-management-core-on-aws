...
import type { DataAssetTasksService } from './api/dataAssetTask/service.js';
import type { NewDataAssetTaskResource } from './api/dataAssetTask/schemas.js';
...
const stepFunctionsEventProcessor = di.resolve<StepFunctionsEventProcessor>('stepFunctionsEventProcessor');
...
export const handler: EventBridgeHandler<string, EventDetails, void> = async (event, _context: Context, _callback: Callback) => {
	...
	else if ((event['detail-type'] as string) === STEP_FUNCTION_STATUS_CHANGE && event['source'] === STEP_FUNCTION_EVENT_SOURCE) {
		await stepFunctionsEventProcessor.processStepFunctionStatusChangeEvent(event as unknown as StepFunctionStateChangeEvent);
	}
	else {
